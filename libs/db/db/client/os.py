...
class OS(Base):
    __tablename__ = "os"
    __table_args__ = (UniqueConstraint('family', 'major', 'minor', 'patch', 'patch_minor', name='unique_os'),)
    ...
    patch_minor = Column(String(10), nullable=True)

    def __str__(self):
        return f"<OS: {self.family}\
        {f' v.{self.major}' if self.major else ''}\
        {f'.{self.minor}' if self.minor else ''}\
        {f'.{self.patch}' if self.patch else ''}\
        {f'.{self.patch_minor}' if self.patch_minor else ''}>"
