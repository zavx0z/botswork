...
const db = new Database("/home/zavx0z/botswork/lib/db/data/northwind.sqlite")
...
export function getCustomers({ offset = 0, limit = 50 }): QueryResult<Customer[]> {
  ...
  const data = stmt.all({ $limit: limit, $offset: offset }) as Customer[]
  return { data, moreRows: getCustomerCount() > offset + limit }
}
...
export function getOrderDetails({ offset = 0, limit = 50 }): QueryResult<OrderDetail[]> {
  ...
  const data = stmt.all({ $limit: limit, $offset: offset }) as OrderDetail[]
  return { data, moreRows: getOrderDetailCount() > offset + limit }
}
