...
export async function initDb(): Promise<InitDbReturn> {
  return new Promise((resolve) => {
    try {
      self.sqlite3InitModule({ print: console.log, printErr: console.error }).then((sqlite3: any) => {
        try {
          ...
          console.log("sqlite3 version", capi.sqlite3_libversion(), capi.sqlite3_sourceid(), `OPFS? ==> ${opfsFound}`)
          if (opfsFound) {
