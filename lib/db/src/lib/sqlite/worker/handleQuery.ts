...
export function handleStorageQuery(msg: WorkerMessage<QueryStorageRequestData>): QueryStorageResponseData {
  try {
    ...
    } else sql = sql.replace("#ORDER_BY#", "order by 1")
    const binds: { [key: string]: number | string } = { $limit: msg.data.limit ?? 999999999 }
    if (msg.data.offset) {
      sql = sql.replace("#LIMIT#", `limit $limit offset $offset`)
      binds["$offset"] = msg.data.offset
    } else sql = sql.replace("#LIMIT#", "limit $limit")
