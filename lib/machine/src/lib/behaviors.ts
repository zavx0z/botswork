import type { ActorContext, ActorRef, Behavior, EventObject, Observer } from "./types"
...
export function fromReducer<TState, TEvent extends EventObject>(
  transition: (state: TState, event: TEvent, actorContext: ActorContext<TEvent, TState>) => TState,
  initialState: TState
...
type PromiseEvents<T> = { type: "fulfill"; data: T } | { type: "reject"; error: unknown }
...
export function fromPromise<T>(promiseFn: () => Promise<T>): Behavior<PromiseEvents<T>, PromiseState<T>> {
