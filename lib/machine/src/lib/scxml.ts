...
function indexedRecord<T extends {}>(items: T[], identifier: string | ((item: T) => string)): Record<string, T> {
  const record: Record<string, T> = {}

  const identifierFn = isString(identifier) ? (item) => item[identifier] : identifier
...
function delayToMs(delay?: string | number): number | undefined {
  ...
  if (secondsMatch) {
    ...
    const millisecondsPart = parseInt((secondsMatch[3] as any).padEnd(3, "0"), 10)
...
function createCond<TContext extends object, TEvent extends EventObject = EventObject>(cond: string) {
...
function toConfig(nodeJson: XMLElement, id: string, options: ScxmlToMachineOptions) {
  ...
  if (nodeJson.elements) {
    ...
    const states: Record<string, any> = indexedRecord(stateElements, (item) => `${item.attributes!.id}`)
...
function scxmlToMachine(scxmlJson: XMLElement, options: ScxmlToMachineOptions): AnyStateMachine {
  const machineElement = scxmlJson.elements!.find((element) => element.name === "scxml") as XMLElement
...
export function toMachine(xml: string, options: ScxmlToMachineOptions): AnyStateMachine {
