...
import { symbolObservable, isMachine, mapContext, toInvokeSource } from "./utils"
...
export function createInvocableActor<TC, TE extends EventObject>(
  ...
  const resolvedData = invokeDefinition.data ? mapContext(invokeDefinition.data, context, _event) : undefined
  const tempActor = serviceCreator
    ? createDeferredActor(serviceCreator as Spawnable, invokeDefinition.id, resolvedData)
...
export function createDeferredActor(entity: Spawnable, id: string, data?: any): ActorRef<any, undefined> {
...
>(actorRefLike: TActorRefLike): ActorRef<TEvent, TEmitted> & Omit<TActorRefLike, keyof ActorRef<any, any>> {
  return {
