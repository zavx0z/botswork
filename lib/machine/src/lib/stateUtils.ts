import type { EventObject, StateValue } from "./types"
...
export function getAllChildren<TC, TE extends EventObject>(stateNode: StateNode<TC, any, TE>): Array<StateNode<TC, any, TE>> {
...
export function getAllStateNodes<TC, TE extends EventObject>(stateNode: StateNode<TC, any, TE, any, any, any>): Array<StateNode<TC, any, TE, any, any, any>> {
...
export function getConfiguration<TC, TE extends EventObject>(
  ...
  for (const s of configuration) {
    ...
    if (s.type === "compound" && (!adjList.get(s) || !adjList.get(s)!.length)) {
      if (prevAdjList.get(s)) prevAdjList.get(s)!.forEach((sn) => configuration.add(sn))
      else s.initialStateNodes.forEach((sn) => configuration.add(sn))
    } else {
      if (s.type === "parallel") {
        for (const child of getChildren(s)) {
          if (!configuration.has(child)) {
            configuration.add(child)
            if (prevAdjList.get(child)) prevAdjList.get(child)!.forEach((sn) => configuration.add(sn))
            else child.initialStateNodes.forEach((sn) => configuration.add(sn))
          }
        }
      }
    }
...
function getValueFromAdj<TC, TE extends EventObject>(baseNode: StateNode<TC, any, TE>, adjList: AdjList<TC, TE>): StateValue {
...
export function getValue<TC, TE extends EventObject>(rootNode: StateNode<TC, any, TE, any>, configuration: Configuration<TC, TE>): StateValue {
  const config = getConfiguration([rootNode], configuration)
  return getValueFromAdj(rootNode, getAdjList(config))
}

export function has<T>(iterable: Iterable<T>, item: T): boolean {
  if (Array.isArray(iterable)) return iterable.some((member) => member === item)
...
export function isInFinalState<TC, TE extends EventObject>(configuration: Array<StateNode<TC, any, TE, any, any, any>>, stateNode: StateNode<TC, any, TE, any, any, any>): boolean {
  if (stateNode.type === "compound") return getChildren(stateNode).some((s) => s.type === "final" && has(configuration, s))
...
export function getTagsFromConfiguration(configuration: StateNode<any, any, any, any>[]) {
