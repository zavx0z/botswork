...
import type { TypegenDisabled } from "./typegenTypes"
...
const validateArrayifiedTransitions = <TContext>(
  ...
  const eventText = event === NULL_EVENT ? "the transient event" : `event '${event}'`
...
> {
   ...
   */
  public doneData?: Mapper<TContext, TEvent, any> | PropertyMapper<TContext, TEvent, any>
  /**
  ...
  private __cache = {
    ...
    transitions: undefined as Array<TransitionDefinition<TContext, TEvent>> | undefined,
    candidates: {} as {
      [K in TEvent["type"] | NullEvent["type"] | "*"]:
        | Array<TransitionDefinition<TContext, K extends TEvent["type"] ? Extract<TEvent, { type: K }> : EventObject>>
        | undefined
    },
    delayedTransitions: undefined as Array<DelayedTransitionDefinition<TContext, TEvent>> | undefined,
  }

  private idMap: Record<string, StateNode<TContext, any, TEvent, any, any, any>> = {}
  public tags: string[] = []
  ...
  constructor(
     ...
     */
    private _context: Readonly<TContext> | (() => Readonly<TContext>) = ("context" in config
    ...
    this.key = this.config.key || _stateInfo?.key || this.config.id || "(machine)"
    this.machine = this.parent ? this.parent.machine : (this as any)
    this.path = this.parent ? this.parent.path.concat(this.key) : []
    this.delimiter = this.config.delimiter || (this.parent ? this.parent.delimiter : STATE_DELIMITER)
    this.id = this.config.id || [this.machine.key, ...this.path].join(this.delimiter)
    ...
    this.schema = this.parent
      ? this.machine.schema
      : (this.config as MachineConfig<TContext, TStateSchema, TEvent>).schema ?? ({} as this["schema"])
    ...
    if (!IS_PRODUCTION) {
      warn(
        ...
        `The "parallel" property is deprecated and will be removed in version 4.1. ${
          this.config.parallel ? `Replace with \`type: 'parallel'\`` : `Use \`type: '${this.type}'\``
    ...
    this.states = (
      this.config.states
        ? mapValues(this.config.states, (stateConfig: StateNodeConfig<TContext, any, TEvent>, key) => {
    ...
    function dfs(stateNode: StateNode<TContext, any, TEvent, any, any, any>): void {
    ...
    this.onEntry = toArray(this.config.entry || this.config.onEntry).map((action) => toActionObject(action as any))
    // TODO: deprecate (exit)
    this.onExit = toArray(this.config.exit || this.config.onExit).map((action) => toActionObject(action as any))
    this.meta = this.config.meta
    this.doneData = this.type === "final" ? (this.config as FinalStateNodeConfig<TContext, TEvent>).data : undefined
  ...
  ): StateNode<TContext, TStateSchema, TEvent, TTypestate, TServiceMap, TResolvedTypesMeta> {
   ...
   */
  public withContext(context: TContext | (() => TContext)): StateNode<TContext, TStateSchema, TEvent, TTypestate> {
  ...
  public get definition(): StateNodeDefinition<TContext, TStateSchema, TEvent> {
    return {
      ...
      states: mapValues(this.states, (state: StateNode<TContext, any, TEvent>) => state.definition) as StatesDefinition<
        TContext,
        TStateSchema,
        TEvent
      >,
   ...
   */
  private getDelayedTransitions(): Array<DelayedTransitionDefinition<TContext, TEvent>> {
    ...
    const mutateEntryExit = (delay: string | number | DelayExpr<TContext, TEvent>, i: number) => {
    ...
    const delayedTransitions = isArray(afterConfig)
      ...
      : flatten(
          Object.keys(afterConfig).map((delay, i) => {
            ...
            const resolvedTransition = isString(configTransition) ? { target: configTransition } : configTransition
  ...
  public getStateNodes(
    state: StateValue | State<TContext, TEvent, any, TTypestate, TResolvedTypesMeta>
  ): Array<StateNode<TContext, any, TEvent, TTypestate, TServiceMap, TResolvedTypesMeta>> {
    if (!state) {
      return []
    }
    ...
    const subStateNodes: Array<StateNode<TContext, any, TEvent, TTypestate, TServiceMap, TResolvedTypesMeta>> = [this]

    subStateNodes.push(
      ...flatten(
        subStateKeys.map((subStateKey) => this.getStateNode(subStateKey).getStateNodes(stateValue[subStateKey]))
      )
    )
  ...
  public resolveState(
    ...
    const stateFromConfig = state instanceof State ? state : State.create(state)

    const configuration = Array.from(getConfiguration([], this.getStateNodes(stateFromConfig.value)))
    return new State({
      ...stateFromConfig,
  ...
  private transitionCompoundNode(
    ...
    const next = stateNode._transition(stateValue[subStateKeys[0]], state, _event)
  ...
  private transitionParallelNode(
    ...
    const stateTransitions = Object.keys(transitionMap).map((key) => transitionMap[key])
    const enabledTransitions = flatten(stateTransitions.map((st) => st.transitions))
  ...
  public getTransitionData(state: State<TContext, TEvent, any, any, any>, event: Event<TEvent> | SCXML.Event<TEvent>) {
  ...
  private next(
      ...
      try {
        ...
        throw new Error(
          `Unable to evaluate guard '${
            cond!.name || cond!.type
          }' in transition for event '${eventName}' in state node '${this.id}':\n${err.message}`
        )
      }
    ...
    return {
      ...
      exitSet: isInternal
        ? []
        : flatten(nextStateNodes.map((targetNode) => this.getPotentiallyReenteringNodes(targetNode))),
  ...
  private getPotentiallyReenteringNodes(
    ...
    let marker: StateNode<TContext, any, TEvent, any, any, any> | undefined = this
    let possibleAncestor: StateNode<TContext, any, TEvent, any, any, any> = targetNode
  ...
  private getActions(
    ...
    const prevConfig = prevState ? getConfiguration([], this.getStateNodes(prevState.value)) : []
    const entrySet = new Set<StateNode<any, any, any, any, any, any>>()

    for (const sn of Array.from(resolvedConfig).sort((a, b) => a.order - b.order)) {
    ...
    const doneEvents = flatten(
      entryStates.map((sn) => {
        ...
        if (grandparent.type === "parallel") {
          if (getChildren(grandparent).every((parentNode) => isInFinalState(transition.configuration, parentNode))) {
    ...
    const entryActions = entryStates
      .map((stateNode) => {
        ...
        const invokeActions = stateNode.activities.map((activity) => start(activity))
        return {
          type: "entry",
          actions: toActionObjects(
            predictableExec ? [...entryActions, ...invokeActions] : [...invokeActions, ...entryActions],
      ...
      .concat({
        type: "state_done",
        actions: doneEvents.map((event) => raise(event as any)) as Array<ActionObject<TContext, TEvent>>,
      })

    const exitActions = Array.from(exitStates).map((stateNode) => ({
      type: "exit",
      actions: toActionObjects(
        [...stateNode.onExit, ...stateNode.activities.map((activity) => stop(activity as any))],
        this.machine.options.actions as any
      ),
    }))
    ...
    const actions = exitActions
      .concat({
        type: "transition",
        actions: toActionObjects(transition.actions, this.machine.options.actions as any),
      })
      .concat(entryActions)

    if (isDone) {
      const stopActions = toActionObjects(
        flatten([...resolvedConfig].sort((a, b) => b.order - a.order).map((stateNode) => stateNode.onExit)),
        this.machine.options.actions as any
   ...
   */
  public transition(
    state: StateValue | State<TContext, TEvent, any, TTypestate, TResolvedTypesMeta> = this.initialState,
    ...
    const stateTransition = this._transition(currentState.value, currentState, _event) || {
      transitions: [],
      configuration: [],
      exitSet: [],
      source: currentState,
    ...
    const prevConfig = getConfiguration([], this.getStateNodes(currentState.value))
  ...
  private resolveRaisedTransition(
    ...
    state = this.transition(state, _event as SCXML.Event<TEvent>, undefined, predictableExec)
  ...
  private resolveTransition(
    ...
    const willTransition = !currentState || stateTransition.transitions.length > 0
    ...
    for (const block of actionBlocks) {
      for (const action of block.actions) {
        if (action.type === actionTypes.start) {
          activities[(action as any).activity!.id || (action as any).activity!.type] = action as ActivityDefinition<
            TContext,
            TEvent
          >
        } else if (action.type === actionTypes.stop) {
          activities[(action as any).activity!.id || (action as any).activity!.type] = false
        }
      }
    }
    ...
    const [raisedEvents, nonRaisedActions] = partition(resolvedActions as any, isRaisableAction)
    ...
    const children = invokeActions.reduce(
      (acc, action) => {
        acc[action.activity.id] = createInvocableActor(action.activity, this.machine as any, updatedContext, _event)

        return acc
      },
      currentState ? { ...currentState.children } : ({} as Record<string, ActorRef<any>>)
    )

    const nextState = new State<TContext, TEvent, TStateSchema, TTypestate, TResolvedTypesMeta>({
   ...
   */
  public getStateNode(stateKey: string): StateNode<TContext, any, TEvent, TTypestate, TServiceMap, TResolvedTypesMeta> {
   ...
   */
  public getStateNodeById(stateId: string): StateNode<TContext, any, TEvent, any, TServiceMap, TResolvedTypesMeta> {
    const resolvedStateId = isStateId(stateId) ? stateId.slice(STATE_IDENTIFIER.length) : stateId
   ...
   */
  public getStateNodeByPath(statePath: string | string[]): StateNode<TContext, any, TEvent, any, any, any> {
    ...
    let currentStateNode: StateNode<TContext, any, TEvent, any, any, any> = this
  ...
  public resolve(stateValue: StateValue): StateValue {
    ...
    switch (this.type) {
      case "parallel":
        return mapValues(this.initialStateValue as Record<string, StateValue>, (subStateValue, subStateKey) => {
          return subStateValue
            ? this.getStateNode(subStateKey).resolve(stateValue[subStateKey] || subStateValue)
            : EMPTY_OBJECT
        })
      ...
      case "compound":
        ...
        return mapValues(stateValue, (subStateValue, subStateKey) => {
          return subStateValue ? this.getStateNode(subStateKey as string).resolve(subStateValue) : EMPTY_OBJECT
        })
  ...
  private getResolvedPath(stateIdentifier: string): string[] {
    if (isStateId(stateIdentifier)) {
      const stateNode = this.machine.idMap[stateIdentifier.slice(STATE_IDENTIFIER.length)]
  ...
  private get initialStateValue(): StateValue | undefined {
    ...
    if (this.type === "parallel") {
      ...
      if (!this.states[this.initial as string]) {
        throw new Error(`Initial state '${this.initial as string}' not found on '${this.key}'`)
      }
      ...
      initialStateValue = (
        isLeafNode(this.states[this.initial as string])
          ? this.initial
          : {
              [this.initial]: this.states[this.initial as string].initialStateValue,
            }
      ) as StateValue
    } else {
   ...
   */
  public get initialState(): State<TContext, TEvent, TStateSchema, TTypestate, TResolvedTypesMeta> {
  ...
  public get target(): StateValue | undefined {
    ...
    if (this.type === "history") {
      const historyConfig = this.config as HistoryStateNodeConfig<TContext, TEvent>
      if (isString(historyConfig.target)) {
        target = isStateId(historyConfig.target)
          ? pathToStateValue(this.machine.getStateNodeById(historyConfig.target).path.slice(this.path.length - 1))
  ...
  public get initialStateNodes(): Array<StateNode<TContext, any, TEvent, any, any, any>> {
    if (isLeafNode(this)) {
      return [this]
    }
    ...
    return flatten(initialStateNodePaths.map((initialPath) => this.getFromRelativePath(initialPath)))
  }
  /**
   ...
   * @param historyValue
   */
  public getFromRelativePath(relativePath: string[]): Array<StateNode<TContext, any, TEvent, any, any, any>> {
  ...
  private historyValue(relativeStateValue?: StateValue | undefined): HistoryValue | undefined {
    ...
    return {
      ...
      states: mapFilterValues<StateNode<TContext, any, TEvent>, HistoryValue | undefined>(
        this.states,
        (stateNode, key) => {
   ...
   * @param historyValue
   */
  private resolveHistory(historyValue?: HistoryValue): Array<StateNode<TContext, any, TEvent, any, any, any>> {
    if (this.type !== "history") {
      return [this]
    }
    ...
    if (!historyValue) {
      ...
      return historyTarget
        ? flatten(toStatePaths(historyTarget).map((relativeChildPath) => parent.getFromRelativePath(relativeChildPath)))
    ...
    return flatten(
      toStatePaths(subHistoryValue!).map((subStatePath) => {
        return this.history === "deep" ? parent.getFromRelativePath(subStatePath) : [parent.states[subStatePath[0]]]
      })
    )
  }
    ...
    return _target.map((target) => {
      ...
      if (this.parent) {
        try {
          ...
          throw new Error(`Invalid transition definition for state node '${this.id}':\n${err.message}`)
        }
      } else {
  ...
  private formatTransition(
    ...
    const internal =
      "internal" in transitionConfig
        ...
        : normalizedTarget
        ? normalizedTarget.some((_target) => isString(_target) && _target[0] === this.delimiter)
        : true
    ...
    const transition = {
      ...
      toJSON: () => ({
        ...transition,
        target: transition.target ? transition.target.map((t) => `#${t.id}`) : undefined,
        source: `#${this.id}`,
      }),
    }
  ...
  private formatTransitions(): Array<TransitionDefinition<TContext, TEvent>> {
    ...
    if (!this.config.on) {
      ...
      const { [WILDCARD]: wildcardConfigs = [], ...strictTransitionConfigs } = this.config.on
      ...
      onConfig = flatten(
        Object.keys(strictTransitionConfigs)
          .map((key) => {
            ...
            const transitionConfigArray = toTransitionConfigArray<TContext, EventObject>(
              key,
              strictTransitionConfigs[key as string]
            )
            if (!IS_PRODUCTION) {
    ...
    const invokeConfig = flatten(
      this.invoke.map((invokeDef: any) => {
        ...
        if (invokeDef.onDone) {
          settleTransitions.push(...toTransitionConfigArray(String(doneInvoke(invokeDef.id)), invokeDef.onDone))
        }
        if (invokeDef.onError) {
          settleTransitions.push(...toTransitionConfigArray(String(error(invokeDef.id)), invokeDef.onError))
        }
    ...
    const formattedTransitions = flatten(
      [...doneConfig, ...invokeConfig, ...onConfig, ...eventlessConfig].map(
        ...
        ) => toArray(transitionConfig).map((transition) => this.formatTransition(transition))
      )
    )
