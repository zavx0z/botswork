...
import type { ActionObject, AnyStateMachine } from "."
...
export function getEventType<TEvent extends EventObject = EventObject>(event: Event<TEvent>): TEvent["type"] {
  try {
    return isString(event) || typeof event === "number" ? `${event}` : (event as TEvent).type
  } catch (e) {
...
export function mapValues(
  ...
  iteratee: (item: unknown, key: string, collection: Record<string, unknown>, i: number) => unknown
) {
...
export function updateHistoryStates(
  ...
  return mapValues(hist.states, (subHist, key) => {
    ...
    const subStateValue =
      (isString(stateValue) ? undefined : stateValue[key]) || (subHist ? subHist.current : undefined)
...
export function toTransitionConfigArray<TContext, TEvent extends EventObject>(
  ...
  configLike: SingleOrArray<TransitionConfig<TContext, TEvent> | TransitionConfigTarget<TContext, TEvent>>
): Array<
...
export function reportUnhandledExceptionOnInvocation(originalError: any, currentError: any, id: string) {
  if (!IS_PRODUCTION) {
    ...
    if (originalError === currentError) {
      ...
      const stackTrace = currentError.stack ? ` Stacktrace was '${currentError.stack}'` : ""
      // tslint:disable-next-line:no-console
      console.error(
...
export function evaluateGuard<TContext, TEvent extends EventObject>(
  ...
  if (guard.type === DEFAULT_GUARD_TYPE) {
    return (guards?.[guard.name] || guard.predicate)(context, _event.data, guardMeta)
  }
...
export function toInvokeSource(src: string | InvokeSourceDefinition): InvokeSourceDefinition {
...
export function isRaisableAction<TContext, TExpressionEvent extends EventObject, TEvent extends EventObject>(
  ...
  return (
    (action.type === actionTypes.raise ||
      (action.type === actionTypes.send &&
        (action as SendActionObject<TContext, TExpressionEvent, TEvent>).to === SpecialTargets.Internal)) &&
    typeof (
      action as
        | RaiseActionObject<TContext, TExpressionEvent, TEvent>
        | SendActionObject<TContext, TExpressionEvent, TEvent>
