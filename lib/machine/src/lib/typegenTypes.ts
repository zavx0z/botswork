...
type GenerateServiceEvent<TServiceName, TEventType, TServiceMap extends ServiceMap> = TEventType extends any
  ? {
      type: TEventType
    } & Prop<TServiceMap, TServiceName>
  : never

type GenerateServiceEvents<TServiceMap extends ServiceMap, TInvokeSrcNameMap> = string extends keyof TServiceMap
  ? never
  : Cast<
      {
    ...
    enabled: TTypesMeta & {
      ...
      indexedEvents: MergeWithInternalEvents<
        IndexByType<
          ...
          | GenerateServiceEvents<TServiceMap, Prop<TTypesMeta, "invokeSrcNameMap">>
        >,
