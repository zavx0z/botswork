import type { ActorRef, AnyActor, AnyStateMachine, Snapshot, SnapshotFrom, StateConfig } from "xstate"
...
export interface Inspector {
  ...
  subscribe: (
    next: (snapshot: SnapshotFrom<ReturnType<typeof createInspectMachine>>) => void,
    error?: (err: any) => void,
    complete?: () => void
  ) => {
    unsubscribe: () => void
 ...
 */
export type ReceiverCommand = { type: "xstate.event"; event: string; service: string } | { type: "xstate.inspecting" }
