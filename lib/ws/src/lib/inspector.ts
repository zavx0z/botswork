import { ActorRef, AnyActor, EventObject, createActor, Observer, toObserver } from "xstate"
...
import { getLazy, isReceiverEvent, parseReceiverEvent, stringify } from "./utils.ts"
...
export function inspect(options?: InspectorOptions): Inspector | undefined {
  ...
  const stringifyWithSerializer = (value: any) => stringify(value, options?.serialize)
...
export function createWindowReceiver(options?: Partial<WindowReceiverOptions>) {
  const { window: ownWindow = window, targetWindow = window.self === window.top ? window.opener : window.parent } =
    options || {}
  ...
  const actorRef = {
    ...
    subscribe(next: (value: ParsedReceiverEvent) => void, error?: (error: any) => void, complete?: () => void) {
