...
/** Converts a state machine or state node to a graph representation.
  *@param {import("$lib/types").MachineJSON} machine - The state machine to convert.
...
export function convertToGraph(machine) {
   ...
   * @param {import("$lib/types").MachineJSON} stateNode - The state machine or state node to convert to a directed graph.
   * @param {string | undefined}  parentID - parent state node id
  */
  function toDirectedGraph(stateNode, parentID) {
    flatten(stateNode.transitions.map((t, transitionIndex) => {
      const targets = t.target || [stateNode.id]
      targets.map((target, idx) => {
        const edgeID = `${stateNode.id}:${transitionIndex}:${idx}`
        edges[edgeID] = {
          id: edgeID,
          source: stateNode.id,
    ...
    nodes[stateNode.id] = {
      ...
      key: stateNode.key,
      type: stateNode.type,
      children: Object.values(stateNode.states).toSorted((a, b) => b.order - a.order).map(state => state.id),
