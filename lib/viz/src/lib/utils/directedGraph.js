/**
 * @typedef { {[key: string]: EdgeTransition} } Edges
 * @typedef { {[key: string]: NodeState} } Nodes
 * @typedef {import("$lib/types").MachineJSON} MachineJSON
 */
...
export function convertToGraph(machine) {
  const /**@type {Edges} */ edges = {}
  const /**@type {Nodes} */ nodes = {}

  /** Generates a directed graph representation of a state machine or state node.
   * @typedef {MachineJSON | import("@lib/machine").StateNodeDefinition<any, any, any>} StateNode
  ...
  */
  function toDirectedGraph(stateNode, parentID) {
    ...
    flatten(stateNode.transitions.map((transition, transitionIndex) =>
