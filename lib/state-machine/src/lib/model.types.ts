...
import type { ResolveTypegenMeta, TypegenConstraint, TypegenDisabled } from "./typegenTypes"

// real `ExtractEvent` breaks `model.assign` inference within transition actions
type SimplisticExtractEvent<TEvent extends EventObject, TEventType extends TEvent["type"]> = TEvent extends {
  type: TEventType
}
  ? TEvent
  : never
...
> {
  ...
  ) => AssignAction<TContext, SimplisticExtractEvent<TEvent, TEventType>, TEvent>
  events: Prop<TModelCreators, "events">
  actions: Prop<TModelCreators, "actions">
  reset: () => AssignAction<TContext, any>
  createMachine: {
    <TServiceMap extends ServiceMap = ServiceMap, TTypesMeta extends TypegenConstraint = TypegenDisabled>(
...
export type ModelContextFrom<TModel extends Model<any, any, any, any>> = TModel extends Model<
  infer TContext,
  any,
  any,
  any
...
export type ModelEventsFrom<TModel extends Model<any, any, any, any> | undefined> = TModel extends Model<
  any,
  infer TEvent,
  any,
  any
...
export type ModelActionsFrom<TModel extends Model<any, any, any, any>> = TModel extends Model<
  any,
  any,
  infer TAction,
  any
...
export type EventCreator<Self extends AnyFunction, Return = ReturnType<Self>> = Return extends object
  ? Return extends {
      type: any
    }
...
export type FinalEventCreators<Self> = {
  [K in keyof Self]: Self[K] extends AnyFunction
    ? (...args: Parameters<Self[K]>) => Compute<ReturnType<Self[K]> & { type: K }>
    : never
}

export type ActionCreator<Self extends AnyFunction, Return = ReturnType<Self>> = Return extends object
  ? Return extends {
      type: any
    }
