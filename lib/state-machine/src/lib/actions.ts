...
export function resolveRaise<TContext, TEvent extends EventObject, TExpressionEvent extends EventObject>(
  ...
  const resolvedEvent = toSCXMLEvent(isFunction(action.event) ? action.event(ctx, _event.data, meta) : action.event)

  let resolvedDelay: number | undefined
  if (isString(action.delay)) {
    const configDelay = delaysMap && delaysMap[action.delay]
    resolvedDelay = isFunction(configDelay) ? configDelay(ctx, _event.data as any, meta as any) : configDelay
  } else {
...
export function resolveSend<TContext, TEvent extends EventObject, TSentEvent extends EventObject>(
 ...
 */
export function sendUpdate<TContext, TEvent extends EventObject>(): SendAction<TContext, TEvent, any> {
  ...
  function handleAction(blockType: string, actionObject: ActionObject<TContext, TEvent>) {
    switch (actionObject.type) {
      ...
      case actionTypes.log: {
        const resolved = resolveLog(actionObject as LogAction<TContext, TEvent>, updatedContext, _event)
      ...
      default:
        let resolvedActionObject = toActionObject(actionObject, machine.options.actions as any)
