...
import type { TypegenDisabled, TypegenEnabled } from "./typegenTypes"
...
> {
  ...
  public history?: State<TContext, TEvent, TStateSchema, TTypestate, TResolvedTypesMeta>
  ...
  public machine: StateMachine<TContext, any, TEvent, TTypestate, BaseActionObject, any, TResolvedTypesMeta> | undefined
  /**
  ...
  constructor(config: StateConfig<TContext, TEvent>) {
    ...
    this.tags = (Array.isArray(config.tags) ? new Set(config.tags) : config.tags) ?? new Set()
   ...
   */
  public toStrings(stateValue: StateValue = this.value, delimiter: string = "."): string[] {
  ...
  public can(event: TEvent | SimpleEventsOf<TEvent>["type"]): boolean {
    ...
    return (
      ...
      transitionData.transitions.some((t) => t.target !== undefined || t.actions.length)
    )
  }
}
