<script lang="ts">
    ...
    import {ButtonCall, ButtonCallEnd} from '@lib/ui/button'
    // import {Io} from 'channels'
    export let data
    let {webRTCReceiver, useMediaDeviceMachine} = data
    const mediaDeviceMachine = webRTCReceiver.children.get('media-device')
    const signalServerMachine = webRTCReceiver.children.get('signal-server')
    // const sio = $signalServerMachine.context.service

    let msg = ''

    // sio.on(Io.CONNECT, (message: any) => {
    //     msg = message
    //     called = true
    // })
...
<p class="text-success-500 text-2xl">
    Сообщение: {msg}
</p>
{#if called}
