...
const Auth = ({user: {login, join, logout}}) =>
    <Routes>
        <Route path={'/' + routes.join.split('/').slice(-1)} element={<Join submit={join} redirect={routes.chat}/>}/>
        <Route path={'/' + routes.login.split('/').slice(-1)} element={<Login submit={login} redirect={routes.chat}/>}/>
