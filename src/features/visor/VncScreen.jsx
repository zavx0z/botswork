...
export default class VncScreen extends Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        const {session, origin, interactive} = this.props

        this.lock(interactive)
    ...
    createRFB(link, port, session, secure) {
        const rfb = new RFB(this.canvas, `${secure ? "wss" : "ws"}://${process.env.REACT_APP_HOST_SELENOID}/vnc/${session}`, {
            credentials: {
                password: "selenoid",
            },
        })
        ...
        rfb.scaleViewport = true
        rfb.resizeSession = true
        rfb.viewOnly = !this.props.interactive
        return rfb
    }
    ...
    render() {
        return <div
            style={{height: this.props.height, width: this.props.width, overflowX: "hidden"}}
            ref={screen => {
                this.canvas = screen
