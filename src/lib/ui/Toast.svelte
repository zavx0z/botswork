<script lang="ts">
	...
	const { toasts, addToast, content, title, description, close, portal } = createToasts<ToastData>({})
...
<button
	class="inline-flex items-center justify-center rounded-md bg-white px-4 py-2 font-medium leading-none text-primary-700 shadow-lg hover:opacity-75"
	...
	{#each $toasts as { id, data } (id)}
		...
		>
			<div class="relative flex w-[24rem] max-w-[calc(100vw-2rem)] items-center justify-between gap-4 p-5">
				...
				<button
					melt={$close(id)}
					class="square-6 absolute right-4 top-4 grid place-items-center rounded-full text-primary-500
