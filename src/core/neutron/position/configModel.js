...
const configModel = types
    .model({
        ...
        frequency: types.maybe(types.number), // Собственная частота (в секундах), которая определяет количество отскоков в секунду при отсутствии демпфирования.
        // Когда определено, tension является производным от this и friction является производным от tension и damping.
        round: types.maybe(types.number), // Во время анимации округлите это число до ближайшего кратного. Это from и to значение никогда не закругленные, а также любое значение , переданное в set метод анимированного значения.
        bounce: types.maybe(types.number), // Когда значение выше нуля, пружина будет подпрыгивать, а не прыгать за пределы своего целевого значения.
        restVelocity: types.maybe(types.number), // Наименьшая скорость перед анимацией считается «неподвижной». Если не определено, precision вместо него используется.
    })
    .actions(self => ({
        setMass(float) {
            self.mass = float
